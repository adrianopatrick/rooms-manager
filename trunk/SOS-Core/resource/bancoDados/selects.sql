...
--seleciona as solicitacoes que nao tem uma reserva associada
select * from solicitacoes s where s.id not in (select r.solicitacao_id from reservas r where r.solicitacao_id = s.id)